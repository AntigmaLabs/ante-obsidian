...
Ante Obsidian is a desktop-only plugin. It still depends on a local Ante Runtime to do actual work, but you no longer need to install `ante` manually before installing the plugin.
...
- Your machine allows Ante Obsidian to check, install, or upgrade the local Ante Runtime from settings.
- If you do not plan to rely on Ante defaults, you have the required provider model and credentials ready.

Ante Obsidian does not become usable just because the plugin files are installed. Before first use, you still need to confirm the local Ante Runtime status in Ante Obsidian settings.
...
6. Enable the plugin.
7. Open the Ante Obsidian settings and confirm the runtime status and provider/model configuration.
...
3. Extract the archive and place the resulting `ante-obsidian/` folder in:
...
5. Open the Ante Obsidian settings and confirm the runtime status and provider/model configuration.
...
### Migrating from older `ante-md` installs

Older builds used the plugin id `ante-md`. Obsidian treats plugin ids as separate plugins, so an existing manual install at:

```text
<your-vault>/.obsidian/plugins/ante-md/
```

will not automatically become `ante-obsidian`.

Before installing the renamed plugin, disable the old plugin, then either remove the old folder or move it to:

```text
<your-vault>/.obsidian/plugins/ante-obsidian/
```

After moving, replace `manifest.json`, `main.js`, and `styles.css` with the files from the new release, then restart Obsidian and enable `Ante Obsidian`.
...
6. Try a simple Ante Obsidian action on a test note.

Ante Obsidian still depends on a working local Ante Runtime, but it now assumes the standard `ante` executable and provides built-in status checks plus install/upgrade actions in settings.
