...
2. Download the plugin release archive (e.g., `ante-obsidian-0.6.0.zip`).
3. Extract the archive and place the `ante-obsidian/` folder in `<your-vault>/.obsidian/plugins/`.
...
Use this option once Ante Obsidian is accepted into the official directory.

1. Open `Settings -> Community plugins`.
2. Select `Browse`, search for `Ante Obsidian`, and install it.
3. Enable the plugin.

## Option 3: Local Install From Source

1. Clone the repository and run `npm install` followed by `npm run build`.
2. Create `<your-vault>/.obsidian/plugins/ante-obsidian/`.
3. Copy `manifest.json`, `main.js`, and `styles.css` into that folder.
4. Enable `Ante Obsidian` in `Settings -> Community plugins`.
