...
本文档介绍如何将 Ante 安装为 Obsidian 插件。

Ante 仅支持 Obsidian 桌面版。插件实际运行依赖本机 Ante Runtime，但你可以在安装插件后，直接从设置页完成 `ante` 的安装。
...
1. 打开 Ante 在 GitHub 上的最新 Release 页面。
2. 下载插件发布压缩包（例如 `ante-0.6.3.zip`），请勿下载源码包。
3. 解压后将得到的 `ante/` 文件夹放到你的 Vault 插件目录下：
   `<your-vault>/.obsidian/plugins/`
4. 打开 Obsidian，在 `Settings -> Community plugins` 中启用 `Ante`。

## 方式二：通过 Obsidian 社区插件市场安装

当 Ante 通过官方审核后可使用此方式。
