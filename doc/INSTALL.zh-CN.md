...
2. 下载插件发布压缩包（例如 `ante-obsidian-0.6.0.zip`），请勿下载源码包。
3. 解压后将得到的 `ante-obsidian/` 文件夹放到你的 Vault 插件目录下：
...
2. 点击 `Browse`，搜索 `Ante Obsidian` 并安装。
3. 启用插件。

## 方式三：从源码本地安装

1. 克隆仓库，依次运行 `npm install` 和 `npm run build`。
2. 在你的 vault 中创建 `<your-vault>/.obsidian/plugins/ante-obsidian/` 目录。
