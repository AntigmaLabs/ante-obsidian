...
export type AnteThinkingPreference =
  | typeof ANTE_DEFAULT_THINKING
  | AnteThinkingLevel;

export const normalizeAnteThinkingPreference = (
  value: unknown
): AnteThinkingPreference =>
  typeof value === "string" &&
  ANTE_THINKING_LEVELS.includes(value as AnteThinkingLevel)
    ? (value as AnteThinkingLevel)
    : ANTE_DEFAULT_THINKING;

export const resolveAnteThinkingPreference = (
  value: AnteThinkingPreference
): AnteThinkingLevel | null =>
  value === ANTE_DEFAULT_THINKING ? null : value;
