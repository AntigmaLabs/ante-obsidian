...
import { renderMissingAnteState, renderSimpleEmptyState } from "./empty-state-renderer";
import { renderChatLayout } from "./chat-layout-renderer";
import { wireChatComposer, wireChatSidebar } from "./chat-shell-wiring";
import { MESSAGE_WINDOW_SIZE, summarizeContext, THINKING_LABELS } from "./chat-view-helpers";
...
export class TmdChatView extends ItemView {
  ...
  async onOpen(): Promise<void> {
    ...
    void this.plugin
      .loadAnteCatalog()
      .then(() => {
        this.viewControls.syncRuntimeTargetControls(
  ...
  private buildShell(): void {
    ...
    wireChatSidebar({
      ...
      onCreateChat: () => {
        ...
        void this.plugin
          ...
          .then((conversation) => {
            this.plugin.chatManager.setConversationRuntimeTarget(conversation.id, runtimeTarget);
          })
          .catch((error) => {
    ...
    this.promptRunner = new ChatPromptRunner(
      ...
      (val) => {
        this.shouldAutoScrollToBottom = val;
      },
      this.composerEl,
    );

    // Instantiate ChatMessageRenderer
    this.messageRenderer = this.addChild(
      new ChatMessageRenderer(this.app, this.plugin, this.timelineEl, {
        getLoadingFrame: () => this.loadingFrame,
        getLiveContext: () => this.liveContext,
        getShouldAutoScrollToBottom: () => this.shouldAutoScrollToBottom,
        setShouldAutoScrollToBottom: (val) => {
          this.shouldAutoScrollToBottom = val;
        },
        shouldStickToBottom: () => this.shouldStickToBottom(),
        scrollToBottom: () => this.scrollToBottom(),
        hasRunningTaskForConversation: (convId) => this.hasRunningTaskForConversation(convId),
        getRefreshPrompt: (msg) => this.promptRunner.getRefreshPrompt(msg),
        refreshMessage: (req) => this.promptRunner.refreshMessage(req),
        triggerRender: () => {
          void this.render();
        },
      }),
  ...
  private syncSidebarCollapsedState(): void {
    ...
    if (this.isSidebarCollapsed) {
      ...
      this.sidebarHeaderEl.insertBefore(this.sidebarToggleEl, this.sidebarHeaderEl.firstChild);
  ...
  private async render(): Promise<void> {
    ...
    const switchedConversation =
      activeConversationId !== null && activeConversationId !== this.lastRenderedConversationId;
    const shouldStickToBottom = this.shouldStickToBottom();
    const shouldForceScrollToBottom = this.shouldAutoScrollToBottom || switchedConversation;
    ...
    this.messageRenderer.syncMessages(
      visibleMessages,
      taskLookup,
      diffsByTask,
    ...
    if (
      ...
      visibleMessages.some((message) => message.id === completedMessageToFocusId)
    ) {
  ...
  private syncContext(context: ContextSnapshot | null): void {
    ...
    const snippetEl =
      existingSnippetEl ?? this.contextEl.createDiv({ cls: "tmd-chat-context-snippet" });
  ...
  private syncLoadMore(conversationId: string | null, hiddenCount: number): void {
    ...
    const button =
      this.loadMoreButtonEl ?? this.timelineEl.createEl("button", { cls: "tmd-chat-load-more" });
