...
import { navigatePromptHistory as computePromptHistoryNavigation } from "../core/terminal-input";
...
export class TmdTerminalView extends ItemView {
  ...
  private render(state: TmdState): void {
    ...
    syncRows(
      this.streamEl,
      this.rowEls,
      buildAllRows(state, this.loadingFrame, this.plugin.shouldShowFullProcessLogs()),
    );
  ...
  private syncPrompt(
    ...
    if (tasks.length === 0) {
      this.editorEl.classList.toggle("tmd-is-empty", this.getEditorText().length === 0);
    }
  }
  ...
  private runPrompt(): void {
    ...
    const tasks = (this.latestState ?? this.plugin.taskEngine.getState()).tasks.filter(
  ...
  private shouldStickToBottom(): boolean {
    const threshold = 24;
    return (
      this.screenEl.scrollTop + this.screenEl.clientHeight >= this.screenEl.scrollHeight - threshold
    );
  }
  ...
  private syncRuntimeHelp(): void {
    ...
    renderMissingAnteState(helpEl, {
      ...
      description: "Open Ante Obsidian Settings to install Ante, then refresh CLI detection here.",
