import { button, div, h2, span, textarea, type ObsidianDomParent } from "./dom-factory";
...
const renderChatHeader = (mainEl: HTMLDivElement): ChatHeaderNodes => {
  const titleRowEl = div({ cls: "tmd-title-row tmd-chat-header" }).appendTo(mainEl);
  const headerActionsEl = div({ cls: "tmd-chat-header-actions" }).appendTo(titleRowEl);
...
const renderChatContext = (mainEl: HTMLDivElement): ChatContextNodes => {
  ...
  const contextTitleEl = div({ cls: "tmd-chat-context-title" }).appendTo(contextEl);
  const contextValueEl = div({ cls: "tmd-chat-context-value" }).appendTo(contextEl);
...
const renderChatComposer = (mainEl: HTMLDivElement): ChatComposerNodes => {
  ...
  const consoleDrawerEl = div({ cls: "tmd-chat-console-drawer" }).appendTo(composerContainerEl);
  ...
  const contextRuntimeEl = div({ cls: "tmd-chat-context-runtime" }).appendTo(composerContainerEl);

  // 3. Create input shell and textarea below
  const inputShellEl = div({ cls: "tmd-chat-input-shell" }).appendTo(composerContainerEl);
  const composerEl = textarea({ cls: "tmd-chat-input" }).appendTo(inputShellEl);
  const attachmentListEl = div({ cls: "tmd-chat-attachments" }).appendTo(inputShellEl);

  // 4. Create composer metadata (bottom bar)
  const composerMetaEl = div({ cls: "tmd-chat-composer-meta" }).appendTo(inputShellEl);
...
export const renderChatLayout = (container: ObsidianDomParent): ChatLayoutNodes => {
  container.empty();
