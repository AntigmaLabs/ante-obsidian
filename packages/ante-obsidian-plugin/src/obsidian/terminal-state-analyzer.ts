...
export const hasStdoutLog = (task: TaskRecord): boolean => task.stdoutText.trim().length > 0;
...
export const summarizeContext = (context: ContextSnapshot | null | undefined): string => {
  if (!context?.filePath) {
...
export const shouldDisplaySystemLog = (text: string, showFullProcessLogs: boolean): boolean =>
...
export const clampTerminalPreview = (value: string, showFullProcessLogs: boolean): string => {
  ...
  const tailLines =
    lines.length > MAX_TERMINAL_PREVIEW_LINES ? lines.slice(-MAX_TERMINAL_PREVIEW_LINES) : lines;
...
export const buildAllRows = (
  ...
  return [...tasks].reverse().flatMap((task) => buildRows(task, loadingFrame, showFullProcessLogs));
};
