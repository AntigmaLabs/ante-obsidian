...
import { normalizeProvider, type AnteProvider } from "./settings";
import {
...
import { THINKING_LABELS } from "./chat-view-helpers";
...
export class ChatViewControls {
  ...
  initializeRuntimeTargetControls(): void {
        ...
        menu.addItem((item) => {
          item
            ...
            .onClick(() => {
              ...
              new ChatProviderSwitchModal(this.app, label, () => {
    ...
    this.modelButtonEl.addEventListener("click", (event) => {
      const menu = new Menu();
      const models = this.plugin.getModelNamesForProvider(
  ...
  async switchProviderConversation(provider: AnteProvider): Promise<void> {
    ...
    try {
      ...
      const conversation = await this.plugin.createChatConversation(this.getLiveContext(), {
        forceNew: true,
      });
  ...
  syncRuntimeTargetControls(conversationId: string | null): void {
    ...
    const provider = normalizeProvider(conversationTarget?.provider ?? fallbackTarget.provider);
    ...
    const thinking = conversationTarget?.thinking ?? this.plugin.settings.anteThinking;
  ...
  resolveConversationSendMode(
    ...
    const currentTarget = this.plugin.chatManager.getConversationRuntimeTarget(conversationId);
    const runtimeSessionId =
      this.plugin.chatManager.getConversationRuntimeSessionId(conversationId);
    const currentThinkingPreference = currentTarget?.thinking ?? this.plugin.settings.anteThinking;
    ...
    if (
      ...
      return {
        ...
        switchedModel: Boolean(currentTarget?.model && currentTarget.model !== target.model),
