...
test("forceNew creates a separate chat even when an empty draft exists", () => {
  const originalWindow = (globalThis as { window?: unknown }).window;
  (globalThis as { window?: unknown }).window = createWindowStub();
  ...
  try {
    ...
    const manager = new ChatSessionManager(pluginStub as never);
    const firstDraft = manager.createConversation({ context });
    const nextDraft = manager.createConversation({ context, forceNew: true });

    assert.notEqual(nextDraft.id, firstDraft.id);
    assert.equal(manager.getActiveConversation().id, nextDraft.id);
  } finally {
    (globalThis as { window?: unknown }).window = originalWindow;
  }
});
